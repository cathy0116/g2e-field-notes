import {parseSource,assemble,esc,str} from './return-core.js';
import {recordHtml,packageReport} from './return-export.js';
...
function render(){if(!batch)return;const q=$('#search').value.trim().toLowerCase(),type=$('#category').value;const rows=batch.records.filter(r=>(!type||r.type===type)&&[r.owner,r.raw.name,r.raw.vendor,r.raw.description,r.raw.personalNotes,r.raw.locationDetail,str(r.raw.notes),str(r.raw.tags)].join(' ').toLowerCase().includes(q));$('#records').innerHTML=rows.slice(0,shown).map(r=>recordHtml(r,batch.attachments,mediaUrl)).join('')||'<p class="empty">沒有符合的紀錄，請調整搜尋或勾選來源。</p>';$('#more').hidden=rows.length<=shown;$('#more').textContent=`顯示更多（目前 ${Math.min(shown,rows.length)} / ${rows.length} 筆）`;}
...
for(const id of ['search','category'])$('#'+id).addEventListener('input',()=>{shown=20;render();});
...
$('#export').addEventListener('click',async()=>{
 if(busy)return;setBusy(true);try{status('正在準備整理包…');batch=assemble(sources);const res=await fetch('./return-template.xlsx');if(!res.ok)throw Error('無法取得 Excel 範本，請連網重試');const bytes=await packageReport(batch,sources,await res.arrayBuffer(),window.JSZip,status);if(downloadUrl)URL.revokeObjectURL(downloadUrl);downloadUrl=URL.createObjectURL(new Blob([bytes],{type:'application/zip'}));const a=document.createElement('a');a.href=downloadUrl;a.download='G2E回國整理-'+new Date().toISOString().replace(/[:.]/g,'-')+'.zip';a.textContent='再次下載整理包';a.className='button';$('#status').textContent='整理包已製作。請確認下載完成，解壓縮後開啟 index.html。 ';$('#status').append(a);a.click();}catch(e){status('匯出失敗：'+e.message+'。原始資料仍保留，可重試。');}finally{setBusy(false);$('#export').disabled=!batch.sources.length;}
});
